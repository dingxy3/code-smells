...
```
   ...
   心理上，通常新建一个新方法的难度超过了在先有方法上去添加改变，你会这样思想斗争着  “它仅仅只需要在老方法改动的地方添加2行代码就可以实现功能， 为了这2行代码创建一个新的方法是没有用的，得不偿失的”  这就意味着另一行代码被添加进去，然后又是另一行代码，以此类推就产生了一团混乱的意大利面条代码
```
...
* 减少一个方法体的长度，使用 提取方法  Extract Method
  ...
  **Solution:**
  ...
  ```
      更加可读的代码！保证看到这个方法的名字就知道你这个方法的目的，形如           creatOrder(),renderCustomerInfo()等等。
